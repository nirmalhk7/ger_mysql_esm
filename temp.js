...
const init_events_table = (knex, schema) => knex.schema.createTable(`${schema}.events`,function(table) {
  table.increments();
  ...
  const i1 = knex.raw(`create index idx_person_created_at_${schema}_events on \`${schema}\`.events (person, action, created_at DESC)`);
  const i2 = knex.raw(`create index idx_thing_created_at_${schema}_events on \`${schema}\`.events (thing, action, created_at DESC)`);
  return bb.all([i1,i2]);
});
...
const drop_tables = function(knex, schema) {
  ...
  return knex.schema.dropTableIfExists(`${schema}.events`)
...
class MysqlESM {
  ...
  list_namespaces() {
    return this._knex.raw("SELECT table_schema as table_schema FROM information_schema.tables WHERE table_name = 'events'")
    .then( function(res) {
      let ret = _.uniq(res[0].map( row => row.table_schema));
  ...
  add_events(events) {
    ...
    for (let e of Array.from(events)) {
      ...
      if (e.expires_at) { e.expires_at = moment(e.expires_at).utc().format('YYYY-MM-DD HH:mm:ss.SSS'); }
  ...
  add_events_to_namespace(namespace, events) {
    return this._knex(`${namespace}.events`).insert(events)
    .catch( error => {
      if (error.errno === 1146) {
        throw new this.NamespaceDoestNotExist();
      }
    });
  }
  ...
  find_events(namespace, options) {
    ...
    if (options.time_until_expiry) { options.expires_after = moment(options.current_datetime).add(options.time_until_expiry, 'seconds').format('YYYY-MM-DD HH:mm:ss.SSS'); }

    let q = this._knex(`${namespace}.events`)
    ...
    if (options.expires_after) { q.where('expires_at', '>', options.expires_after); }

    if (options.person) { q = q.where({person: options.person}); }
    if (options.people) { q = q.whereIn('person', options.people); }

    if (options.action) { q = q.where({action: options.action}); }
    if (options.actions) { q = q.whereIn('action', options.actions); }

    if (options.thing) { q = q.where({thing: options.thing}); }
    if (options.things) { q = q.whereIn('thing', options.things); }
  ...
  thing_neighbourhood(namespace, thing, actions, options) {
    if (options == null) { options = {}; }
    if (!actions || (actions.length === 0)) { return bb.try( () => []); }
    ...
    const one_degree_away = this._one_degree_away(namespace, 'thing', 'person', thing, actions, options)
  ...
  person_neighbourhood(namespace, person, actions, options) {
    ...
    const one_degree_away = this._one_degree_away(namespace, 'person', 'thing', person, actions, options)
  ...
  _unexpired_events(namespace, actions, options) {
    return this._knex(`${namespace}.events`)
    .select('person')
  ...
  _one_degree_away(namespace, column1, column2, value, actions, options) {
    ...
    const recent_events = this._knex(`${namespace}.events`)
    .where(query_hash)
    .whereIn('action', actions)
    ...
    .innerJoin(`${namespace}.events as f`, function() { return this.on(`e.${column2}`, `f.${column2}`).on(`f.${column1}`,'!=', `e.${column1}`); })
    ...
    .select(this._knex.raw(`f.${column1}, MAX(f.created_at) as last_actioned_at, MAX(f.expires_at) as last_expires_at, group_concat(distinct f.${column2}) as ${column2}, DATE(max(e.created_at)) as created_at_day, count(f.${column1}) as action_count`))
    .groupBy(`f.${column1}`);
  }
  ...
  filter_things_by_previous_actions(namespace, person, things, actions) {
    if (!actions || (actions.length === 0) || (things.length === 0)) { return bb.try(() => things); }
    ...
    const filter_things_sql = this._knex(`${namespace}.events`)
    .select("thing")
  ...
  _recent_events(namespace, column1, actions, values, options) {
    if (options == null) { options = {}; }
    if ((values.length === 0) || (actions.length === 0)) { return bb.try(() => []); }
    ...
    const expires_after = moment(options.current_datetime).add(options.time_until_expiry, 'seconds').format();
...
where created_at <= :now and action in (${action_values}) and ${column1} = :${key} and (expires_at > :expires_after ) group by person, thing order by last_actioned_at DESC limit ${options.recommendations_per_neighbour})`
    ...
    if (ql.length > 1) { query += " order by last_actioned_at DESC"; }
  ...
  cosine_distance(namespace, column1, column2, limit, a_values, al_values, vkey) {
    const s1q = this._history(namespace, column1, column2, ':value', al_values, limit).toString();
    const s2q = this._history(namespace, column1, column2, `:${vkey}`, al_values, limit).toString();
  ...
  get_cosine_distances(namespace, column1, column2, value, values, actions, limit, event_decay_rate, now) {
    ...
    if (values.length === 0) { return bb.try( () => []); }
    ...
    for (action in actions) {
      ...
      action_list.push({action, weight, weight});
    }
    ...
    for (let vi = 0; vi < values.length; vi++) {
      ...
      cosine_distance = this.cosine_distance(namespace, column1, column2, limit, a_values, al_values, vkey);
  ...
  _similarities(namespace, column1, column2, value, values, actions, options) {
    if (options == null) { options = {}; }
    if (!actions || (actions.length === 0) || (values.length === 0)) { return bb.try(() => ({})); }
    options = _.defaults(options, {
  ...
  count_events(namespace) {
    return this._knex(`${namespace}.events`).count()
    .then(count => parseInt(count[0]['count(*)']));
  }

  estimate_event_count(namespace) {
    return this._knex.raw(`SELECT table_rows as estimate \
from information_schema.tables \
WHERE table_name = 'events' and table_schema = :ns`
      ,{ns: `${namespace}`})
    .then( function(ret) {
      if (ret[0].length === 0) { return 0; }
  ...
  analyze(namespace) {
    return this._knex.raw(`ANALYZE TABLE \`${namespace}\`.events`);
  }
  ...
  compact_people(namespace, compact_database_person_action_limit, actions) {
    return this.get_active_people(namespace)
    .then( people => {
      return this.truncate_people_per_action(namespace, people, compact_database_person_action_limit, actions);
    });
  }

  compact_things(namespace, compact_database_thing_action_limit, actions) {
    return this.get_active_things(namespace)
    .then( things => {
      return this.truncate_things_per_action(namespace, things, compact_database_thing_action_limit, actions);
    });
  }
  ...
  truncate_things_per_action(namespace, things, trunc_size, actions) {
    ...
    if (things.length === 0) { return bb.try( () => []); }
    ...
    for (let thing of Array.from(things)) {
      for (let action of Array.from(actions)) {
        ((thing, action) => {
          return promise = promise.then(() => this.truncate_thing_actions(namespace, thing, trunc_size, action) );
        })(thing, action);
      }
    }
  ...
  truncate_people_per_action(namespace, people, trunc_size, actions) {
    //TODO do the same thing for things
    if (people.length === 0) { return bb.try( () => []); }
    ...
    for (let person of Array.from(people)) {
      for (let action of Array.from(actions)) {
        ((person, action) => {
          return promise = promise.then(() => this.truncate_person_actions(namespace, person, trunc_size, action) );
        })(person, action);
      }
    }
  ...
  remove_events_till_size(namespace, number_of_events) {
    ...
    const query = `delete from \`${namespace}\`.events where id not in (select id from \`${namespace}\`.events order by created_at desc limit ${number_of_events})`;
